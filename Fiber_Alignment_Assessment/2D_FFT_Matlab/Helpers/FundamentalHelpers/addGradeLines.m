...
%             end
            ...
            index = unique(index);
           % index = sortIndexes( index,middleWidth,middleHeight,size(img) );
    ...
    function [ new_indexes ] = onlyCloseIndexes( indexes,middleWidth_L,middleHeight_L,image_size )
        
        limit =  round(middleWidth_L*0.75);
         new_indexes = [];
         
        for indi = indexes
            [a,b] = ind2sub(image_size,indi);
            distance = pdist([middleHeight_L,middleWidth_L;a,b], 'euclidean');
            if (distance <limit)
                new_indexes = [new_indexes, indi];
            end
        end
